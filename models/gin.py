...
class UnsupervisedGIN(nn.Module):
    ...
    def __init__(
        ...
        ---------
        num_layers: int
            The number of linear layers in the neural network (INCLUDING the input layer)
        num_mlp_layers: int
            The number of linear layers in mlps (EXCLUDING the input layer)
        input_dim: int
            The dimensionality of input features
        hidden_dim: int
            The dimensionality of hidden units at ALL layers
    ...
    def forward(self, g, h, efeat):
        ...
        for i, h in list(enumerate(hidden_rep)):
            pooled_h = self.pool(g, h) # (subgraph_size, c) --> (1, c)
