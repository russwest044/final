...
parser.add_argument('--auc_test_rounds', type=int, default=10)
	...
	with tqdm(total=args.num_epoch) as pbar:
		...
		for epoch in range(args.num_epoch):
			...
			for batch_idx in range(batch_num):
				...
				ba_hat = []
				bf = []
				added_adj_zero_row = torch.zeros((cur_batch_size, 1, subgraph_size)).to(device) # (B, 1, S)
				added_adj_zero_col = torch.zeros((cur_batch_size, subgraph_size + 1, 1)).to(device) # (B, S+1, 1)
				added_adj_zero_col[:, -1, :] = 1.
				added_feat_zero_row = torch.zeros((cur_batch_size, 1, ft_size)).to(device) # (B, 1, C)
				...
				NCE_loss1 = get_NCEloss(subgraph_embed, subgraph_embed_hat, cur_batch_size)
				NCE_loss2 = get_NCEloss(subgraph_embed, node_embed_hat, cur_batch_size)
				# gamma = 0.1
				NCE_loss = NCE_loss2
...
def test():
	...
	# 	'./pretrained/{}.pkl'.format(args.dataset)))
		...
		for round in range(args.auc_test_rounds):
			...
			for batch_idx in range(batch_num):
				...
				added_adj_zero_row = torch.zeros(
					(cur_batch_size, 1, subgraph_size)).to(device)
				added_adj_zero_col = torch.zeros(
					(cur_batch_size, subgraph_size + 1, 1)).to(device)
				added_adj_zero_col[:, -1, :] = 1.
				added_feat_zero_row = torch.zeros(
					(cur_batch_size, 1, ft_size)).to(device)
				for i in idx:
					cur_adj = adj[:, subgraphs[i], :][:, :, subgraphs[i]]
					cur_adj_hat = adj_hat[:, subgraphs[i], :][:, :, subgraphs[i]]
				...
				with torch.no_grad():
					...
					test_logits_1 = torch.sigmoid(
						torch.squeeze(test_logits_1))
					test_logits_2 = torch.sigmoid(
						torch.squeeze(test_logits_2))
					test_logits_1_hat = torch.sigmoid(
						torch.squeeze(test_logits_1_hat))
					test_logits_2_hat = torch.sigmoid(
						torch.squeeze(test_logits_2_hat))
					...
					ano_score = args.beta * (args.alpha * ano_score_1 + (1 - args.alpha) * ano_score_1_hat) + \
						(1 - args.beta) * (args.alpha * ano_score_2 +
											(1 - args.alpha) * ano_score_2_hat)

				multi_round_ano_score[round, idx] = ano_score

			pbar_test.update(1)

		ano_score_final = np.mean(
			multi_round_ano_score, axis=0) + np.std(multi_round_ano_score, axis=0)
		auc = roc_auc_score(ano_labels, ano_score_final)
		all_auc.append(auc)
		print('Testing AUC:{:.4f}'.format(auc), flush=True)


def get_NCEloss(embed, embed_hat, cur_batch_size):
	# subgraph-subgraph contrast loss
	embed = F.normalize(embed, dim=1, p=2) # (B, C)
	embed_hat = F.normalize(embed_hat, dim=1, p=2) # (B, C)
	sim_matrix_one = torch.matmul(embed, embed_hat.t()) # (B,C)*(C,B) --> (B, B)
	sim_matrix_two = torch.matmul(embed, embed.t())
	sim_matrix_three = torch.matmul(embed_hat, embed_hat.t())
	temperature = 1.0
	...
	nega_list = np.arange(0, cur_batch_size - 1, 1) # [0, 1, 2, ..., B-2]
	nega_list = np.insert(nega_list, 0, cur_batch_size - 1) # [B-1, 0, 1, 2, ..., B-2]
...
if __name__ == '__main__':
	...
	for run in range(args.runs):
		...
		adj, features, labels, ano_labels, str_ano_labels, attr_ano_labels = load_mat(args.dataset)
		...
		model = Model(
			...
			args.embedding_dim,
			'prelu',
			args.negsamp_ratio_patch,
			args.negsamp_ratio_context,
			args.readout
		).to(device)
		...
		b_xent_patch = nn.BCEWithLogitsLoss(
			reduction='none',
			pos_weight=torch.tensor([args.negsamp_ratio_patch]).to(device)
		)
		b_xent_context = nn.BCEWithLogitsLoss(
			reduction='none',
